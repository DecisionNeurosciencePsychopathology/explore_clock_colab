...
get_trial_data_test <- function(repo_directory=NULL, dataset="explore_clock", groupfixed=TRUE) {
  ...
  if(dataset=="explore_clock") {
    ...
    trial_df <- read.csv('~/explore_clock//explore_decay_factorize_selective_psequate_fixedparams_fmri_ffx_sceptic_trial_statistics_new.csv')
    trial_df %>%
      group_by(id) %>%
      mutate(run_number = case_when(trial <= 40 ~ 1, 
                                    trial <= 80 & trial > 40 ~ 2, 
                                    trial <= 120 & trial > 80 ~ 3,
                                    trial <= 160 & trial > 120 ~ 4,
                                    trial <= 200 & trial > 160 ~ 5,
                                    trial <= 240 & trial > 200 ~ 6)) -> trial_df
  } else if (dataset=="mmclock_fmri") {
    trials_per_run <- 50
...
if (dataset == "bsocial_clock") {
  trial_df <- trial_df %>%
    rename(id=ID, rt_csv=rt, score_csv=score, run_number=run)
}

  trial_df <- trial_df %>%
    
    arrange(id, run_number, run_trial) %>% # make sure trials are consecutive
  ...
  trial_df <- trial_df %>% mutate(trial_neg_inv_sc = scale(trial_neg_inv))
